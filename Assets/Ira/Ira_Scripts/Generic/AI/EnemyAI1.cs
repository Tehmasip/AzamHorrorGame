...
public class EnemyAI1 : MonoBehaviour
{
    ...
    private int _currentWaypoint = 0;//Current Position which teacher have to reach

    private Animator _animator;//Animator of Teacher

    private bool _roundCompleted = false;//The round of covering all the wayPoints

    [SerializeField] EnemyStates _currentState;// THe state in which the teacher is currently in
    ...
    void Update()
    {
        if (transform.gameObject.activeSelf)
        {
            ...
            {
                if(!GamePlayHandler.Instance.PlayerDie)
                {
            ...
            if (Vector3.Distance(transform.position, _objectToChase.position) <= CatchDistance&&!GamePlayHandler.Instance.PlayerDie&&EnemyController.Instance.IsTowardsPlayer)//Distance Between Player and Teacher
            {
                ...
                GamePlayHandler.Instance.PlayerRevivePos.position = GamePlayHandler.Instance.Player.transform.position;
                GamePlayHandler.Instance.PlayerRevivePos.rotation = GamePlayHandler.Instance.Player.transform.rotation;
